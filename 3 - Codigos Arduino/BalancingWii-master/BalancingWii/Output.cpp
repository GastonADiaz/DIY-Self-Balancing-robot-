...
#define LED_FRENTE A2    // LED para ambos motores adelante
#define LED_ATRAS A2     // LED para ambos motores atrás
#define LED_DERECHA A0   // LED para motor 1 derecha y motor 2 izquierda
#define LED_IZQUIERDA A1  // LED para motor 1 izquierda y motor 2 derecha
#define CLR(x,y) (x&=(~(1<<y)))
#define SET(x,y) (x|=(1<<y))
volatile uint32_t pulseCountMotor1 = 0;
volatile uint32_t pulseCountMotor2 = 0;
...
ISR(TIMER1_COMPA_vect) // DRIVER_PIN[5] = {5,6,7,8,4};   //STEP1 (PORTD 5), STEP2 (PORTD 6), DIR1 (PORTD 7), DIR2 (PORTB 0), ENABLE
{
  ...
  periodsCounter[1]++;
   ...
   // Apagar todos los LEDs al principio de la ISR
  ...
  // Activar LEDs según la dirección de los motores
  if (actualMotorDir[0] == actualMotorDir[1]) {
    if (actualMotorDir[0] == 0) {
      digitalWrite(LED_FRENTE, HIGH);  // Ambos motores adelante
    } else {
      digitalWrite(LED_ATRAS, HIGH);   // Ambos motores atrás
    }
  }
}
